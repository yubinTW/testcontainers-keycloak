...
import { Keycloak24Container, StartedKeycloakContainer } from '../src/keycloak24-container'

describe('Keycloak24 Container Test', () => {
  let keycloak: StartedKeycloakContainer

  beforeAll(async () => {
    keycloak = await new Keycloak24Container()
  ...
  it('user operations: create, get, set-password', async () => {
    await keycloak.createUser('demo', 'user01', 'yubin', 'hsu', 'demo@test.com', true)
